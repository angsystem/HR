(function () {
  ...
  function handleAuthFailed(event) {
    ...
    if (getPendingPlan()) {
      window.setTimeout(function () {
        /* 查無帳號只代表切換為註冊意圖；不得偽造 ANG_HR_AUTH_VERIFIED。 */
  ...
  function installConstructionProviders() {
    var card = getLoginCard();
    if (!card) return;
    var apple = card.querySelector('.apple-login, [data-provider="apple"]');
    if (!apple) return;
    apple.disabled = false;
    apple.setAttribute('aria-disabled', 'true');
    apple.setAttribute('title', 'Apple 建構中');
    apple.dataset.construction = 'true';
    var label = apple.querySelector('span');
    if (label) label.textContent = 'Apple｜建構中';
  }

  function blockConstructionProvider(event) {
    var button = event.target && event.target.closest ? event.target.closest('[data-construction="true"]') : null;
    if (!button) return;
    event.preventDefault();
    event.stopImmediatePropagation();
    var card = getLoginCard();
    if (card) {
      card.classList.remove('login-system-confirmed', 'login-identifier-valid');
      var status = card.querySelector('.login-auth-status');
      if (status) {
        status.className = 'login-auth-status info';
        status.textContent = 'Apple 登入建構中，尚未開放。';
      }
    }
  }
  ...
  function start() {
    ...
    syncAuthDecision();
    observeEntry();
    installConstructionProviders();
  }

  document.addEventListener('pointerdown', onOwnedPointerDown, true);
  document.addEventListener('click', blockConstructionProvider, true);
